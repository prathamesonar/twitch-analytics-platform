...
export default function TitleTester({ channelLogin }) {
  ...
  useEffect(() => {
    const fetchTitles = async () => {
      try {
        setLoading(true);
        const { data } = await api.get(`/api/channels/${channelLogin}/title-testing`);
        setTitles(data || []);
      } catch (err) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg h-full">
      <h3 className="text-xl font-bold mb-4">Title/Thumbnail Testing</h3>
      <div className="space-y-3 h-64 overflow-y-auto pr-2">
        {loading ? (
          <p className="text-gray-500">Analyzing titles...</p>
        ) : titles.length === 0 ? (
          <p className="text-sm text-gray-500">
            Not enough data yet. (Needs 30 days of metrics)
          </p>
        ) : (
          titles.map((stream, index) => (
            <div key={index} className="p-3 bg-gray-100 dark:bg-gray-700 rounded">
              ...
              <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
                <span>Avg: {stream.avg_viewers.toLocaleString()}</span>
